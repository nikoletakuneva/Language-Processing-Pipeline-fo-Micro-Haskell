...
class MH_Typechecker {
	...
	throws TypeError, UnknownVariable {
        ...
        // add code here
    	if (exp.isVAR()){
    		MH_TYPE t = env.typeOf(exp.value());
    		return t;
    	} 
    	else if (exp.isNUM()){
    		return IntegerType;
    		...
    		MH_EXP first = exp.first();
    		MH_EXP second = exp.second();
    		
    		if (computeType(first, env).left().equals(computeType (second, env))){
    			// computeType(first, env).left() is the type of the argument of the function
    			return computeType(first, env).right(); // type of the result of the function
    		}
    		else {
    			throw new TypeError ("Argument doesn't match function type.") ;
    		}
    		
    	}
    	else if (exp.isINFIX()){
    		if (!computeType(exp.first(), env).equals(IntegerType) || !computeType(exp.second(), env).equals(IntegerType)){
    			throw new TypeError ("Expression is not of type Integer.") ;
    		}
    			...
    			throw new TypeError ("Unknown infix operator.") ;
    		}
    	}
    	else if (exp.isIF()){
    		if (!computeType(exp.first(), env).equals(BoolType)){
    			throw new TypeError ("If-condition is not of type Bool.") ;
    		}
    		else if (!computeType(exp.second(), env).equals(computeType(exp.third(), env))){
    			// The type of the expression after then doesn't match the type of the expression after else
    			throw new TypeError ("Type mismatch in if-statement.") ;
    		}
    		else {
    			return computeType(exp.second(), env); // == computeType(exp.third(), env)
    		}
    	}
    	else {
